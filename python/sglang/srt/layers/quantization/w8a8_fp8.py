...
# AVO: route per-channel (PTPC) MoE through aiter asm fused_moe(per_Token) on
# fnuz HIP (gfx942). The upstream Triton per-channel fnuz MoE path produces
# corrupted output on gfx942 (offline ref-diff: aiter cos=0.999 vs bf16 ref).
...
class W8A8Fp8LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self.quantization_config.is_checkpoint_fp8_serialized:
            ...
            if _use_aiter:
                # AVO P1b: MFMA-preshuffle B so gemm_a8w8_bpreshuffle gets the fast layout.
    ...
    def apply(
        ...
        return apply_fp8_linear(
            ...
            # AVO P1: on aiter/HIP route per-token-per-channel fp8 to the fused
            # aiter rowwise GEMM (gemm_a8w8_bpreshuffle) instead of the unfused
            # _apply_fallback_scaled_mm (fp8->fp32 GEMM + separate dequant+cast).
